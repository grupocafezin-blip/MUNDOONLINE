...
(() => {
  ...
  // Simple game helpers/state
  window.Game = window.Game || {
    state: { projectiles: [], lastMoveAngle: 0, aimPhase: 0, aimDir: 1 },
    ensurePlayer(p){
      p.inventory = p.inventory || { fruits:{}, page:0, perPage:8 };
...
// expose utility to plugins
window.world = window.world || world;
window.__tileAtProxy = window.__tileAtProxy || tileAt;
  ...
  function connect() {
    ...
    window.socket = socket; // expose for plugins
    window.socket = socket; // expose socket for plugins
    ...
    socket.on("inventoryUpdate", (u)=>{
      if (u.id === me.id){
        me.coins = u.coins|0;
        me.weapon = u.weapon||null;
        me.inventory = me.inventory || { fruits:{}, page:0, perPage:8 };
        me.inventory.fruits = u.fruits || {};
        const coinsEl = document.getElementById('coinsVal'); if (coinsEl) coinsEl.textContent = String(me.coins||0);
        const vCoins = document.getElementById('vCoins'); if (vCoins) vCoins.textContent = String(me.coins||0);
      } else {
        const p = players.get(u.id);
        if (p){ p.weapon = u.weapon||p.weapon; }
      }
    });
    socket.on("weaponChanged", ({id, weapon})=>{
      const p = players.get(id); if (p){ p.weapon = weapon; if (id===me.id) me.weapon = weapon; }
    });
  ...
  function startGame(data){
    ...
    window.world = window.world || world;
    window.__tileAtProxy = window.__tileAtProxy || tileAt;
    if (window.Game && window.Game.useHookInit) try{ window.Game.useHookInit(); }catch(e){}

    // mark game as running
    playing = true;
  } = data);
    me = window.Game.ensurePlayer(data.you); window.me = me;
    config = data.config || config;
    players.clear();
    for (const p of data.players) players.set(p.id, p);
    players.set(me.id, me);
