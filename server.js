...
io.on("connection", (socket)=>{
  ...
  socket.on("collectItem", ({ itemId }) => {
    ...
    if ((dx*dx + dy*dy) <= (28*28)){
      it.taken = true;
      if (it.type === 'fruit') p.fruits = (p.fruits|0) + 1;
      else if (it.type === 'coin') p.money = (p.money|0) + 1;
      p.score = (p.score|0) + 1;
      io.to(roomId).emit("itemTaken", { itemId, by: p.id, score: p.score, fruits: p.fruits|0, money: p.money|0 });
      if (p.score % 10 === 0){
